...
public class RejectFragment extends Fragment implements APIcall.ApiCallListner{
    ...
    public void onSuccess(int operationCode, String response, Object customData) {
        if (operationCode == APIcall.OPERATION_GROUP_USER_LIST) {
            ...
            if (mgruopMaster.getStatus() != 0) {
                ...
                List<GroupDatum>  mRGroupData = new ArrayList<>();
                mGroupData = new ArrayList<>();
                mGroupData = mgruopMaster.getData();

                for(int i = 0; i < mgruopMaster.getData().size(); i++){
                    if(mGroupData.get(i).getIsApproved() == 2){
                        mRGroupData.add(mGroupData.get(i));
                    }
                }
                if(mRGroupData.size() == 0){
                    recycler_view.setVisibility(View.GONE);
                    ll_no_data.setVisibility(View.VISIBLE);
                }else {
//                    groupUserListAdapter = new RVGroupUserListAdapter(activity, mRGroupData);
